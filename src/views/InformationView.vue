<template>
  <div>
    <section class="info1">
      ...
      <button @click="startQuestionnaire(0)">Commencer le questionnaire</button>
    </section>
    <div v-if="showQuestionnaire && currentQuestionIndex !== null">
    ...
    <section class="info2">
      ...
      <button @click="startQuestionnaire(1)">Commencer le questionnaire</button>
    </section>
    <div v-if="showQuestionnaire && currentQuestionIndex !== null">
      <h1>{{ currentQuestion.title }}</h1>
      <p>{{ currentQuestion.description }}</p>
    ...
    <section class="info3">
      ...
      <button @click="startQuestionnaire(2)">Commencer le questionnaire</button>
    </section>
    <div v-if="showQuestionnaire && currentQuestionIndex !== null">
      <h1>{{ currentQuestion.title }}</h1>
      <p>{{ currentQuestion.description }}</p>
...
let showQuestionnaire = ref(false);
let currentQuestionIndex = ref(null);
let selectedOption = ref(null);

const questionsInfo1 = [
  // Questions de la première section
  [
    {
      title: "Quête 1/4",
...
const questionsInfo2 = [
  [
    ...
    // Ajoutez les autres questions de la deuxième section ici
  ],
];

function envoyerScore() {
  const currentQuestion = getCurrentQuestion();
  const questions = currentQuestionIndex.value < 4 ? questionsInfo1 : questionsInfo2;
  ...
  if (currentQuestionIndex.value + 1 < questions[sectionIndex].length) {
    ...
    currentQuestionIndex.value += 4; // Passer à la première question de la section suivante
  } else {
...
function startQuestionnaire(sectionIndex) {
  showQuestionnaire.value = true;
  currentQuestionIndex.value = sectionIndex * 4; // Indice de la première question de la section
}
</script>
<style lang="scss">
